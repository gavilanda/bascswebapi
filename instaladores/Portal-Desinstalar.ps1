# ============================================================
#  Portal-Desinstalar.ps1
#  Elimina TODO lo instalado del Portal de Clientes:
#   - Cierra el tray si esta corriendo
#   - Quita la tarea programada
#   - Detiene y borra el servicio
#   - Borra la regla de firewall
#  NO toca la base de datos ni los archivos publicados.
...
# ---- 1) Cerrar el proceso del tray (PowerShell que ejecuta Portal-Tray.ps1) ----
$proc = Get-CimInstance Win32_Process -Filter "Name = 'powershell.exe'" -ErrorAction SilentlyContinue |
        Where-Object { $_.CommandLine -like "*Portal-Tray.ps1*" }
if ($proc) {
...
if (Get-NetFirewallRule -DisplayName "PortalClientes $Port" -ErrorAction SilentlyContinue) {
    Remove-NetFirewallRule -DisplayName "PortalClientes $Port"
...
Write-Host ""
Write-Host "Desinstalacion completa. (La base de datos y los archivos publicados quedan intactos.)" -ForegroundColor Cyan
